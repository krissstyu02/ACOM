...
while True:
    ...
    upper_red = np.array([10, 255, 255]) # максимальные значения оттенка, насыщенности и значения(яркости)
    ...
    # вычисление моментов на основе маски(характеристик)
    moments = cv2.moments(mask)

    # поиск момента первого порядка- общая площадь объекта, который был выделен маской на изображении
    area = moments['m00']

    if area > 0:
        # ширина и высота прямоугольника равны квадратному корню из площади объекта
        width = height = int(np.sqrt(area))
        # вычисление координат центра объекта на изображении с использованием моментов первого порядка
